...
Nu skapar jag ett par kataloger för att ge en grundstruktur till webbplatsen. Tanken är att katalogen `public/` innehåller allt som behöver vara publikt och tillgängligt på webbplatsen och övriga kataloger är sådant som inte behöver vara tillgängligt via webbservern. Det är en bra start.
...
```console title="Hämta hem bilder från nätet via en webblänk och spara lokalt"
...
```console title="Katalogstruktur när du är klar"
$ tree .
.                           
├── config                  
│   └── config.php          
