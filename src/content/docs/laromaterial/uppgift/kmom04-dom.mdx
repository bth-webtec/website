...
I den här uppgiften tränar vi mer på manipuleringen av domträdet. Vi ska bygga en sida där vi skapar och manipulerar element. Vi ska även skapa eventlysnare som hjälper oss med den dynamiska delen.
...
2. Skapa filen `public/js/dom.js` och läs in den i `dom.html`. Det är "startfilen" för din JavaScript.
...
Skapa en låda på din sida. Lådan ska vara centrerad med en storlek på 200 x 200 pixlar. Du kan styla lådan på valfritt sätt.

Avsluta med att skapa en funktion i JavaScript som hjälper dig att skriva ut följande via `console.log()`.
...
Fånga eventet för när webbläsarens fönster ändrar storlek. När det händer skall du alltid placera din låda i mitten av fönstret.
...
Koppla eventhanteraren till din låda.
...
När man trycker på `p` så skall ett slumpmässigt element skapas. Ta en kopia av din box, men slumpa fram dess position, färg och form.
